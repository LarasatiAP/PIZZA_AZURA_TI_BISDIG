...
2. **Fullstack Mode (Server-side)**: Run via a Node.js Express server (`server.js`) with an SQLite database (`sql.js`) to provide secure admin authentication, persistent relational data storage, and unified server routing.
...
*   **`/db.js`**: Database connection and initialization using `sql.js` (SQLite).
*   **`/pizza_azura.db`**: Relational database storage file.

### Architecture Comparison
| Feature | Frontend-Only Mode (Root) | Fullstack Mode (Server) |
| :--- | :--- | :--- |
| **Data Storage** | Browser `localStorage` | SQLite File Database (`pizza_azura.db`) |
...
The system defines the following relational structures, implemented directly in SQLite and mirrored in the `localStorage` JSON structures:
